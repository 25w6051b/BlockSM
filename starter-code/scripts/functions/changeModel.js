    ...
    if (input && input.connection) {
        ...
        if (connected) {
            if(connected.type == "initialStateType"){
                ...
                inputStateName = "FinalState";
            }
        }
    }
    if (inputStateName == "undefined") {
        pbJson.push({
    ...
    if (input && input.connection) {
        ...
        if (connected) {
            ...
            if (connected.type == "initialStateType"){
                ...
                let inputValueIf2 = connected.getFieldValue("IF2") || "<color: red>undefined<color: black>";
                ...
                let computationList = ["and","or"]; // 取得した値から該当する等号を取得
                let operator = computationList[parseInt(inputValueIf1.replace("VALUE", ""))];
                result = `${inputValueIf0} ${operator} ${inputValueIf2}`;
            }
                ...
                // IF部分が2つあるため個別に取得
                let inputValueIf0 = exploreIfBlocks(connected,"IF0");
                let inputValueIf1 = connected.getFieldValue("IF1") || "<color: red>undefined<color: black>";
                result = `${inputValueIf0} is not ${inputValueIf1}`;
            }
        ...
        // ここではif部分にブロックが挿入されていない場合に，エラーハンドリングを行う(具体的には赤色で未定義と表示させる)
            ...
            if (block.type == "changeStateType"){
                result = "undefined" + `${undefinedStateNumber}`; // plantUMLの形はこの形式とする(赤色にするのはサーバー側で行う)
                undefinedStateNumber += 1;
            }
            else {
                result = "<color: red>undefined<color: black>" // plantUMLの形はこの形式とする(赤色で表示)
            }
            // skipModelConversion = true;
        }
    } 
    else {
        console.log("The exploreIfBlocks function can only be called on blocks that contain an if component.");
        // skipModelConversion = true;
    }
...
function getNameByTypeIndex(text, blockType, namesArray) {
    ...
    if (match && match[1]) {
        ...
        console.warn(`"${text}" did not match the pattern ^${blockType}(\\d+)$`);
           ...
           if (connected.type == "entryType" || connected.type == "doContinuousType" || connected.type == "doOnetimeType" || connected.type == "exitType"){
                ...
                if (childInput && childInput.connection) {
                    ...
                    if (childConnected != null){
                        ...
                        console.log("No action block has been set");
                        let inputValue = "<color: red>undefined<color: black>"; // 動きブロックが配置されていないため，「未定義」を追加
                        pbJsonPushDetail(stateName, connected.type,inputValue,index); // pbJsonにブロックの情報を追加
                        // skipModelConversion = true;
                    }
...
function exploreBehaviorNextBlocks(block,stateName,index){
    ...
    if(nextBlock){
        ...
        if (childInput && childInput.connection) {
            ...
            if (childBlock != null){
                const inputValue = getNameByTypeIndex(childBlock.type,"behavior",behaviorNames); // 接続されたブロックの名称を取得
...
function pbJsonPushDetail(stateName, parentType, detailValue,index) {
    ...
    if (target) {
        ...
        console.warn(stateName + "does not exist"); // 状態名が存在しない場合
    }
}
    ...
    pbJson.forEach(stateObj => {
        ...
        if (Array.isArray(transitions) && transitions.length > 0) {
            transitions.forEach(tr => {
                ...
                if (tr.nextState == undefined && state != "InitialState") {
                    tr.nextState = "undefined" + undefinedStateNumber;
                    undefinedStateNumber += 1;
                }
            });
        }
...
function findErrorNoteInPlantUML(){
    ...
    if (/InitialStateDefinitionError/.test(plantUmlText) ){
        ...
        if(lastDefLogged == false){
            logToTxt('Diagram conversion error: Please start with "InitialState"');
            lastDefLogged = true;
    ...
    // 初期状態からの遷移が定義されていないときのエラー
        ...
        if(lastTransLogged == false){
            logToTxt('Diagram conversion error: Please specify the next state following "InitialState"');
            lastDefLogged = false;
