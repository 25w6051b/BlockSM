...
for (let i = 0; i < effectNames.length; i++){
  ...
  if(valueMatch !=  null){
    variableNames.push(valueMatch[1]);
  }
  else{
...
javascript.javascriptGenerator.forBlock["valueChangeType"] = function(block) {
  ...
  if (parentBlock != null){
    ...
    checkForChildBlock(block,"IF0",'Please enter a variable into the "=" block');
    if(isNaN(conditionCode2)){
      code += console.log('Enter numbers only into the "=" block');
                logError('Enter numbers only into the "=" block',logDiv);
    }
